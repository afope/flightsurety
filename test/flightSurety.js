...
it('Registered airlines can pay dues', async function () {
    const payAirlineDues = await config.flightSuretyApp.payAirlineDues({ from: secondAirline, value: web3.toWei(10, 'ether') });
    await config.flightSuretyApp.payAirlineDues({ from: thirdAirline, value: web3.toWei(10, 'ether') });
    await config.flightSuretyApp.payAirlineDues({ from: fourthAirline, value: web3.toWei(10, 'ether') });
...
it('Passenger can buy insurance for flight', async function () {
    ...
    const insurance = await config.flightSuretyApp.getInsurance(flight1.flight);
    console.log('insurance', insurance)
    console.log('expectedPayoutAmount', expectedPayoutAmount)
